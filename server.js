...
async function getMovieBoxStream(type, tmdbId, season, episode, hevcSupport = true) {
  try {
    ...
    // Helper: check if we should transcode locally or if forced on production
    const isLocal = process.platform === 'win32' || process.env.LOCAL_DEV === 'true';
    const isTranscodingEnabled = isChosenHevc && !hevcSupport && (isLocal || process.env.ENABLE_TRANSCODING === 'true');
    ...
    function makeStreamUrl(rawUrl, codec) {
      ...
      if (isHevc && !hevcSupport) {
        if (isLocal || process.env.ENABLE_TRANSCODING === 'true') {
  ...
  try {
    ...
    if (movieBoxResult) {
      ...
      if (!isHevc || hevcSupport || movieBoxResult.transcoded) {
        ...
        console.log(`[MovieBox] Skipped HEVC stream for movie ${tmdbId} (client requested H.264 only, transcoding unavailable)`);
      }
    }
  } catch (err) {
  ...
  try {
    ...
    if (movieBoxResult) {
      ...
      if (!isHevc || hevcSupport || movieBoxResult.transcoded) {
        ...
        console.log(`[MovieBox] Skipped HEVC stream for TV ${tmdbId} S${season}E${episode} (client requested H.264 only, transcoding unavailable)`);
      }
    }
  } catch (err) {
    console.error('MovieBox fetch error:', err);
