...
export async function renderMoviesPage({ container, query }) {
  ...
  // Load initial data
  await loadMovies(container, false);
...
async function loadMovies(container, append = false) {
  if (isLoading) return;
  isLoading = true;
  ...
  try {
    ...
    if (!append && grid) {
      grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 40px; color: var(--text-muted);">Failed to load movies.</div>';
    }
  }
